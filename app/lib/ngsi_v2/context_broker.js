...
class ContextBroker {
    ...
    getExistingSubscriptions(brokerUrl, params) {
        const paramsString = REST.stringifyUrlParamsObject(params);
        const headers = this.setRequestHeaders({'Accept': 'application/json'});
        return this.REQ_LIMITER.schedule(() => REST.executeRequest('GET', `${brokerUrl}/subscriptions${paramsString}`, headers, null));
    }

    createSubscription(brokerUrl, entityType) {
        if (!entityType) {
            logger.error('createSubscription: no entity type given - NGSI v2 subscription won\'t be created');
            return;
        }

        const headers = this.setRequestHeaders({'Content-Type': 'application/json'});
        const notificationRequestHeaders = this.setQLRequestHeaders();
        ...
        return this.REQ_LIMITER.schedule(() => REST.executeRequest('POST', `${brokerUrl}/subscriptions`, headers, JSON.stringify(subscription)));
    }
    ...
    updateEntities(brokerUrl, entities) {
        ...
        if (batchBodySize > 1048576) {
            ...
            return this.REQ_LIMITER.schedule(() => REST.executeRequest('POST', `${brokerUrl}/op/update`, headers, batchBody));
        }
    }
    ...
    async createOrUpdateEntities(brokerUrl, entities) {
        ...
        try {
            ...
            const existingEntitiesResponse = await this.getExistingEntities(brokerUrl, {type: entityType, attrs: 'id', options: 'keyValues,count', limit: '1000'});
            ...
            if (existingEntitiesResponse && existingEntitiesResponse.data && Array.isArray(existingEntitiesResponse.data)) {
                ...
                if (entitiesToUpdate.length || entitiesToCreate.length) {
                    ...
                    if (process.env.ENABLE_HISTORIC_DATA_STORAGE && process.env.ENABLE_HISTORIC_DATA_STORAGE === 'true' && process.env.QL_V2_NOTIFICATION_BASE_URL) {
                        ...
                        // than specified query parameter 'limit' (1000 is max. number allowed by FIWARE Orion Context Broker implementation) -> assume we don't exceed this limit for now
                        const existingSubscriptionsResponse = await this.getExistingSubscriptions(brokerUrl, {options: 'count', limit: '1000'});
                        if (existingSubscriptionsResponse && existingSubscriptionsResponse.data && Array.isArray(existingSubscriptionsResponse.data)) {
                            ...
                            if (!subscriptionExists) {
                                ...
                                // create subscription
                                await this.createSubscription(brokerUrl, entityType);
                            }
                        }
                    }
