...
setTimeout(() => {
    ...
    async function queryVBBStations() {
        try {
            ...
            // code still needs some throttling mechanism to handle this amount of data in parallel.
            ...
            // <--
            const stations = await VBB_REST.allStations();
            ...
            logger.error('queryVBBStations: awaiting VBB_REST.allStations: ' + error);
        }
    }
    ...
    async function queryVBBJourneys(/*stations*/) {
        ...
        const queryResults = journeyParams.map(paramObject => VBB_REST.journeys(paramObject));
        return Promise.all(queryResults.map(async (result) => {
            try {
                ...
                logger.error('queryVBBJourneys: awaiting VBB_REST.journeys: ' + error);
            }
        }));
    }
                    ...
                    await Promise.all(journey.legs.map(async (route) => {
                        ...
                        if (line) {
                            ...
                            if (operator) {
                                ...
                                const trip = await VBB_REST.trip(route.tripId, {
                                    lineName: line.name,
