...
setTimeout(() => {
    ...
    function removeObjectDuplicates(objArray, objIdentifier) {
        if (!Array.isArray(objArray)) {
            logger.error('removeObjectDuplicates: objArray is not an array');
            return objArray;
        }
        if (!objIdentifier) {
            logger.error('removeObjectDuplicates: unsupported object identifier ' + objIdentifier);
            return objArray;
        }

        // return a reduced subset of the given object array with all duplicates of an object (identified by its given identifier) removed
        return objArray.reduce((accumulated, currItem) => accumulated.find(accuTtem => accuTtem[objIdentifier] === currItem[objIdentifier]) ? [...accumulated] : [...accumulated, currItem], []);
    }
    ...
    async function queryVBBStations() {
        try {
            ...
            // TODO --> due to runtime reasons, instead of querying all stations (~ 13000) currently an static array of 10 arbitrary stations is returned for demo purposes.
    ...
    async function processJourneys(journeys) {
        try {
            ...
            const transformedNGSIagencies = transformIntoNGSI(removeObjectDuplicates(gtfsAgencies, 'id'), 'GtfsAgency');
            const transformedNGSIroutes = transformIntoNGSI(removeObjectDuplicates(gtfsRoutes, 'id'), 'GtfsRoute');
            const transformedNGSIshapes = transformIntoNGSI(removeObjectDuplicates(gtfsShapes, 'id'), 'GtfsShape');
    ...
    async function queryAndImportData() {
        try {
            ...
            const transformedNGSIstations = transformIntoNGSI(removeObjectDuplicates(stations, 'id'), 'GtfsStation');
            const transformedNGSIstops = transformIntoNGSI(removeObjectDuplicates(stops, 'id'), 'GtfsStop');
